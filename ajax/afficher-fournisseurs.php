...
foreach ($conn->query($sql) as $row) { // Loop through each row and for each row display table layout
  print '<tr>';
  print '<td><a href="index.php?page=modifier-fournisseur&id='.$row['id_fournisseur'].'">'.ucwords(strtolower($row['nom_fournisseur'])).'</a> 
  </br></br>'.$row['adresse'].'<br>'.$row['code_postal'].' '.strtoupper($row['ville']).'</br></br>
  <span class="secondary radius label">'.$row['id_fournisseur'].'</span></td>'; // Name of supplier and ID label
  print '<td><strong>'.$row['personne_contact'].'</strong></td>';
